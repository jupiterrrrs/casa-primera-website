# Casa Primera - apply chatbot + navbar fixes directly (no patch needed)
# Run this from the ROOT of your repo (Casa-Primera-Website folder)

$ErrorActionPreference = "Stop"

Write-Host "Writing src/app/components/LiveChat.tsx ..."
@'
...
'@ | Set-Content -Path "src/app/components/LiveChat.tsx" -Encoding UTF8 -NoNewline

Write-Host "Writing src/app/components/Navbar.tsx ..."
@'
...
'@ | Set-Content -Path "src/app/components/Navbar.tsx" -Encoding UTF8 -NoNewline

Write-Host "Files written. Showing git diff summary:"
git diff --stat

Write-Host "Staging, committing, and pushing..."
git add src/app/components/LiveChat.tsx src/app/components/Navbar.tsx
git commit -m "Fix AI chatbox location answer/matching and keep mobile Book Now button visible"
